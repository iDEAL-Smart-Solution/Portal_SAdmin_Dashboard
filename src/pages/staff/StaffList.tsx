...
const StaffList: React.FC<StaffListProps> = ({ onViewProfile, onEditStaff, onAddStaff }) => {
  ...
  const [sortBy, setSortBy] = useState<'name' | 'date' | 'uin'>('name');
  ...
  const filteredAndSortedStaff = React.useMemo(() => {
    let filtered = staffList.filter(staff => {
      const matchesSearch = staff.fullName.toLowerCase().includes(searchTerm.toLowerCase()) ||
                           ...
                           staff.UIN.toLowerCase().includes(searchTerm.toLowerCase()) ||
                           staff.userName.toLowerCase().includes(searchTerm.toLowerCase());
      
      const matchesGender = filterGender === 'all' || staff.gender === filterGender;
    ...
    return filtered;
  }, [staffList, searchTerm, filterGender, sortBy]);
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-6">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <div>
            ...
            <select
              id="sort"
              value={sortBy}
              onChange={(e) => setSortBy(e.target.value as 'name' | 'date' | 'uin')}
              className="block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500"
            >
        ...
        {/* Clear Filters */}
        {(searchTerm || filterGender !== 'all' || sortBy !== 'name') && (
            ...
            <div className="flex items-center">
              ...
              <div className="ml-5 w-0 flex-1">
                <dl>
                  ...
                  <dd className="text-lg font-medium text-gray-900">{filteredAndSortedStaff.length}</dd>
                </dl>
              </div>
            </div>
          </div>
